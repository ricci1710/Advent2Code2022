...
import {MOCK_DATA_DAY_04} from "./data04";
import {MOCK_DEMO_DATA_DAY_04} from "./demo04";
...
const Logic04 = () => {
    ...
    const [data] = useState(MOCK_DATA_DAY_04.split('\n'));
    const [demoData] = useState(MOCK_DEMO_DATA_DAY_04.split('\n'));

    const prepareData = (value) => {
        const pair = value.split(',');
        const first = getMinMaxPair(pair[0]);
        const second = getMinMaxPair(pair[1]);
        return {first, second};
    };
    ...
    const getMinMaxPair = (value) => {
        const pair = value.split('-');
        return { min: parseInt(pair[0],10), max: parseInt(pair[1], 10) };
    };
    ...
    const checkIsOverlap = (first, second) => {
        // check is first contain in second?
        // either Min in range of 2. container or max in range of 2. container.
        const isFirstOverlapSecond =  (first.min >= second.min && first.min <= second.max);
        if (isFirstOverlapSecond)
            return true;

        return  (second.min >= first.min && second.min <= first.max);
    };
    ...
    const calcPartOne = (valueAsArray) => {
        ...
        valueAsArray.forEach((item) => {
            const {first,second} = prepareData(item);
            const inRange = checkInRange(first, second);
            if (inRange)
                result += 1;
        });
        return result;
    ...
    const calcPartTwo = (valueAsArray) => {
        ...
        valueAsArray.forEach((item) => {
            const {first,second} = prepareData(item);
            const isOverlap = checkIsOverlap(first, second);
            if (isOverlap)
                result += 1;
        });
        return result;
    ...
    console.assert(demoScore === 2, `Algorithm is incorrect - expected: 2 calculated value: ${demoScore}`);
    console.log('Demo-Score (Part One)  -> 157 ===', demoScore);
    const score = calcPartOne(data);
    console.log('Score (Part One)  -> ??? (424) ===', score);
