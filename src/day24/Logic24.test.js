import {MOCK_DEMO_DATA_DAY_24} from "../day24/demo24";
...
describe('Test Class Day24', () => {
  test('constructor', () => {
    ...
    expect(7).toEqual(size.x);
    expect(7).toEqual(size.y);
  });
  test('getRowLine', () => {
    const demoData = MOCK_DEMO_DATA_DAY_24.split('\n');
    const day24 = new Day24(demoData);
    const rowLine = day24.playBoard.getRowLine(4);
    expect(7).toEqual(rowLine.length);
  ...
  test('getColumnLine', () => {
    ...
    expect(7).toEqual(columnLine.length);
    expect('#...v.#').toEqual(Array2d.lineToString(columnLine));
  });
});