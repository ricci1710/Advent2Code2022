import {MOCK_DATA_DAY_19} from "./data19";
import {MOCK_DEMO_DATA_DAY_19} from "./demo19";
 ...
 * is 24. Finally, if you add up the quality levels of all the blueprints in the list, you get 33.
 *
...
const Logic19 = () => {
  ...
  const data = MOCK_DATA_DAY_19.split('\n');
  const demoData = MOCK_DEMO_DATA_DAY_19.split('\n');

  console.log(demoData);

  // endregion prepare mock data
  // region score rules
  const readRobotRules = (values) => {
    const robotRules = new Map();
    values.forEach((blueprintRules) => {
      const split1 = blueprintRules.split(': ')
      const key = split1[0];

      const robots = split1[1].split('. ');
      const rules = new Map();
      robots.forEach((robotInfo) => {
        const robotInfoParts = robotInfo.split(' ');
        const robotType = robotInfoParts[1];
        switch (robotType) {
          case 'ore':
          case 'clay':
            rules.set(robotType, {'ore': robotInfoParts[4]});
            break;
          case 'obsidian':
            rules.set(robotType, {'ore': robotInfoParts[4], 'clay': robotInfoParts[7]});
            break;
          case 'geode':
            rules.set(robotType, {'ore': robotInfoParts[4], 'obsidian': robotInfoParts[7]});
            break;
          default:
            break;
        }
      });
      robotRules.set(key, rules);
      console.log(robotRules);
    });
    return robotRules;
  }
  // endregion score rules
  // region score calculation
  const calcPartOne = (values, timerCounter = 24) => {
    ...
    readRobotRules(values);

    // For-Schleife pro Blueprint oder in der For-Schleife alle Blueprints.
    for (let timer = 1; timer <= timerCounter; timer += 1) {

      timeLine.set(timer, robots);

    }

    // quality level of each blueprint by multiplying that blueprint's ID number
